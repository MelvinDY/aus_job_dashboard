...
**Nobody has opened the Power BI report since the rebind.** Claude has no GUI, so this is
...
1. **Real Power BI Desktop screenshots** (blocked on the refresh above).
   ...
   README image. Overview and State are the strongest hero shots. **Claude cannot do this.**
...
- Chart colours are `#3B82C4` / `#C2410C`, not the report's `#1F4E79`. The brand blue fails
  ...
  palette with the `dataviz` skill's `validate_palette.js`; do not eyeball it.
...
- **No AI attribution in commits.** No `Co-Authored-By`, no `Claude-Session` trailer. Ever.
